...
<script type="text/javascript">
    ...
    function allClick() {
        ...
        $("input[name='ck']").each(function (){
            this.checked = flag;
        }); //通过name定位选择到五个name为ck的复选框，然后用each方法来遍历
    }
    ...
    //得到下面五个复选框被选中的个数
        var checkedLength = $("input[name='ck']:checked").length;   //:checked是过滤器
    //进行对比，改变全选复选框的状态
        if(fiveLength == checkedLength){
            $("#all").prop("checked",true)     //通过prop函数对checked属性进行赋值,相等就是true
        }else{
...
<script type="text/javascript">
    ...
    function deleteBatch() {
        ...
        //如果有选中的商品，则获取其value的值进行字符串拼接
        if(cks.length == 0){
            alert("请先选择将要删除的商品！");
        }else {
            var str = "";
            var id = "";
            if(confirm("您确定要删除这"+cks.length+"条商品吗")){
                // alert("可以进行删除！")
                //进行提交商品id字符串的拼接
                $.each(cks,function (){
            ...
            $.ajax({
                ...
                success:function (msg){
                    alert(msg);  //内容其实就是批量删除成功或者批量删除失败或者是不可删除
                    //将页面上显示商品数据的容器重新加载
    ...
    function  del(pid,page) {
        ...
        if(confirm("您确定要删除吗？")){
            ...
            $.ajax({
                ...
                success:function (msg){
                    alert(msg);
                    //刷新table容器:选择器选择table容器，然后用load方法刷新
                    $("#table").load("http://localhost:8080/admin/product.jsp #table");
...
<script type="text/javascript">
    function ajaxsplit(page) {
        ...
        //向服务器端发送Ajax请求，请求page页中的所有数据，在当前页面上局部刷新显示
        $.ajax({
            url:"${pageContext.request.contextPath}/prod/ajaxSplit.action",
            data:{"page":page,"pname":pname,"typeid":typeid,"lprice":lprice,"hprice":hprice},
            type:"post",
            success:function (){
                //重新加载显示分页数据的容器  #table的意思是在你前面这个页面上的table容器重新加载一遍
                $("#table").load("http://localhost:8080/admin/product.jsp #table");
            }
        });
